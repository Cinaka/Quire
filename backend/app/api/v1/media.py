...
MIME_SUFFIX = {"image/jpeg": ".jpg", "image/png": ".png", "image/webp": ".webp", "image/gif": ".gif"}
...
def to_response(row: Media) -> MediaResponse:
    return MediaResponse(
        ...
        url=row.url, thumb_url=row.thumb_url or "",
    )


@router.post("/{media_id}")
async def upload_media(
    media_id: uuid.UUID, file: UploadFile = File(...), thumb: UploadFile | None = File(default=None),
    entry_id: uuid.UUID | None = Form(default=None), sort_order: int = Form(default=0),
    ...
    existing = await db.scalar(select(Media).where(Media.id == media_id, Media.user_id == user.id))
    ...
    suffix = MIME_SUFFIX[file.content_type]
    relative_dir = Path(f"{media_id.hex[:4]}")
    ...
    if thumb is not None:
        ...
        if len(thumb_data) <= MAX_BYTES:
            thumb_suffix = MIME_SUFFIX.get(thumb.content_type or "", suffix)
            (disk_dir / f"{media_id}_thumb{thumb_suffix}").write_bytes(thumb_data)
            thumb_url = f"{settings.MEDIA_PUBLIC_PREFIX}/{relative_dir.as_posix()}/{media_id}_thumb{thumb_suffix}"
    ...
    row.entry_id = entry_id; row.sort_order = sort_order; row.width = width; row.height = height
    row.size = len(data); row.url = url; row.thumb_url = thumb_url
    await db.commit(); await db.refresh(row)
    return ok(to_response(row))


@router.get("")
async def list_media(entry_id: uuid.UUID | None = None, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> Envelope[list[MediaResponse]]:
    query = select(Media).where(Media.user_id == user.id)
    if entry_id: query = query.where(Media.entry_id == entry_id)
    rows = list((await db.execute(query.order_by(Media.created_at))).scalars())
    return ok([to_response(row) for row in rows])


@router.delete("/{media_id}")
async def delete_media(media_id: uuid.UUID, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> Envelope[None]:
    row = await db.scalar(select(Media).where(Media.id == media_id, Media.user_id == user.id))
    if row is None: raise HTTPException(status_code=404, detail="图片不存在")
