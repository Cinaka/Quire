...
function comparator(order: EntryOrder): (a: Entry, b: Entry) => number {
  return (a, b) => {
    ...
    const d = order === "entryDateAsc" ? a.entryDate.localeCompare(b.entryDate) : b.entryDate.localeCompare(a.entryDate)
...
export const localEntryRepo: IEntryRepo = {
  async list(params = {}) {
    ...
    let coll = params.dateFrom || params.dateTo
      ? db.entries.where("[isDeleted+entryDate]").between([flag, params.dateFrom ?? "0000-01-01"], [flag, params.dateTo ?? "9999-12-31"], true, true)
    ...
    if (params.hasImage !== undefined) coll = coll.and((entry) => (collectMediaIds(entry.content?.doc).length > 0) === params.hasImage)
    if (params.keyword) {
    ...
    return { items: all.slice((page - 1) * pageSize, page * pageSize).map(stripContent), total: all.length, page, pageSize }
  },
  ...
  async create(dto) {
    ...
    const entry: Entry = {
      id: newId(), entryDate, sortOrder: await nextSortOrder(entryDate),
      title: (dto.title ?? "").slice(0, 255), content, contentText: toPlainText(content),
      mood: dto.mood ?? null, weather: dto.weather ?? null, tagIds: dto.tagIds ?? [],
      fromScheduleId: dto.fromScheduleId ?? null, createdAt: now, updatedAt: now,
      clientUpdatedAt: now, serverUpdatedAt: "", deletedAt: null, isDeleted: 0, dirty: 1,
    }
  ...
  async update(id, dto) {
    return db.transaction("rw", db.entries, async () => {
      ...
      if (dto.content !== undefined) { next.content = dto.content; next.contentText = toPlainText(dto.content) }
      next.updatedAt = utcNow(); next.clientUpdatedAt = next.updatedAt; next.dirty = 1
  ...
  async remove(id) {
    const now = utcNow()
    await db.entries.update(id, { isDeleted: 1, deletedAt: now, updatedAt: now, clientUpdatedAt: now, dirty: 1 })
  },
  async restore(id) {
    const now = utcNow()
    await db.entries.update(id, { isDeleted: 0, deletedAt: null, updatedAt: now, clientUpdatedAt: now, dirty: 1 })
  },
  async purge(id) {
    const cur = await db.entries.get(id)
    if (!cur) return
    ...
    await db.transaction("rw", db.entries, db.media, async () => {
      await db.media.where("entryId").equals(id).modify({ entryId: "", orphanedAt: now, dirty: 1 })
      await db.entries.put({ ...cur, title: "", content: null, contentText: "", mood: null, weather: null, tagIds: [], isDeleted: 1, deletedAt: cur.deletedAt ?? now, updatedAt: now, clientUpdatedAt: now, dirty: 1 })
    })
  },
  async countByDate(from, to) {
    const rows = await db.entries.where("[isDeleted+entryDate]").between([0, from], [0, to], true, true).toArray()
