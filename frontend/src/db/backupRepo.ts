...
function blobToBase64(blob: Blob): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      // readAsDataURL 给的是 "data:image/png;base64,xxxx"
      // 只保留逗号后面的 base64 部分
 ...
 *
 * 主要用于兼容 v1 备份：
 * v1 没有 thumbMime，因此缩略图不能简单使用原图的 rest.mime。
 */
...
function remapMediaRefs(
  ...
  for (const [from, to] of map) {
    json = json
      .split(`${LOCAL_MEDIA_PREFIX}${from}`)
      .join(`${LOCAL_MEDIA_PREFIX}${to}`)
  }
...
export const localBackupRepo = {
  ...
  async exportAll(
    ...
    for (let i = 0; i < mediaIds.length; i++) {
      ...
      media.push({
        ...
        // 缩略图
        // 新版本同时保存 thumbMime，避免导入时错误使用原图 MIME。
        thumbBase64: thumbBlob
  ...
  async importAll(
    raw: unknown,
    conflict: ImportConflict = "preferNewer",
    ...
    //
    // 非常重要：
    // asCopy 时 Entry 和 Media 必须使用同一份 entryIdMap。
    // 否则：
    //
    //   Entry -> 新 ID
    //   Media -> 原 Entry ID
    ...
    // Media ID 映射
    //
    // asCopy 时图片也必须生成新的 media id。
    // 同时正文中的 local://media/{id} 也要同步替换。
    // ─────────────────────────────────────────────
    ...
    for (const bm of file.media) {
      ...
      decoded.push({
        ...
        // asCopy 时必须使用和 Entry 相同的 entryIdMap。
        //
        // 原来的代码在 Media 解码阶段没有提前建立
        // Entry 映射，因此会出现：
        //
        //   新 Entry -> newEntryId
        //   新 Media -> oldEntryId
    ...
    // 构造最终 Entry。
    //
    // id 必须从 entryIdMap 获取，
    // 不能在这里再次 newId()。
    // 否则 Media 使用的 entryIdMap 又会失效。
    // ─────────────────────────────────────────────
    ...
    for (const e of file.entries) {
      ...
      incoming.push({
        ...
        // asCopy 时使用预先建立好的 Entry ID。
        id: entryIdMap.get(e.id) ?? e.id,

        // 正文中的 local://media/{oldId}
        // 同步改成新的 media id。
        content: remapMediaRefs(
          e.content,
          mediaIdMap,
        ),

        // 标签 ID 使用归并后的 ID；多个同名来源标签可能映射到同一 ID，必须去重。
        tagIds: [
          ...new Set(
            (e.tagIds ?? []).map(
              (t) => tagIdMap.get(t) ?? t,
    ...
    // 第三段：事务外对账。
    //
    // 必须放在 Dexie transaction 结束之后。
    //
    // reconcileAll 会按照最终落库的 Entry JSON
    // 校正 media.entryId / sortOrder。
    //
    // 这样导入完成后立刻执行彻底删除，
    // 也不会因为旧的 media.entryId 关联而误删
    // 原条目或副本条目的图片。
    // ─────────────────────────────────────────────
